...
function getProducts(request, response) {
  ...
  let data = []
  if (category_id > 0) {
    const sqlOpdracht = db.prepare('SELECT * FROM products JOIN broek ON products.genre_id = genre.id WHERE category_id = ? ORDER BY id ASC')
    data = sqlOpdracht.all(category_id)
  } else { 
    const sqlOpdracht = db.prepare(' SELECT * FROM products JOIN products ON products.genre_id = genre.id ORDER BY id ASC;')
